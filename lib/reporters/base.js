...
exports.list = function(failures){
  ...
  failures.forEach(function(test, i){
    ...
    if (sameType(actual, expected) && ('string' === typeof actual || Array.isArray(actual) || (actual && typeof actual === 'object'))) {
      fmt = color('error title', '  %s) %s:\n%s') + color('error stack', '\n%s\n');
      var match = message.match(/^([^:]+): expected/);
      msg = match ? '\n      ' + color('error message', match[1]) : '';

      if (Array.isArray(actual) || (actual && typeof actual === 'object')) {
        msg += require('difflet-papandreou')({indent: 4, comment: true}).compare(expected, actual);
      } else if (exports.inlineDiffs) {
